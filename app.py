...
def withdrawal():
    ...
    if (int(amount)<=int(balance)):
        ...
        db.withdrawal.insert_one(
            {
                "Transaction_id": session['account_number'][0:5] + time.strftime("%Y%m%d") + session['account_number'][6:10],
...
def deposit():
    ...
    if (int(amount)>=500):
        ...
        db.deposit.insert_one(
            {
                "Transaction_id": session['account_number'][0:5] + time.strftime("%Y%m%d%H%M%S") + session['account_number'][6:10],
...
def transfer():
    ...
    if int(amount) <= 0:
        return render_template('transferResult.html', result=4)
    ...
    if int(balance) < int(amount):
        ...
        db.transfer.insert_one(
            {
                "Transaction_id": account_number2[0:5] + time.strftime("%Y%m%d%H%M%S") + account_number2[6:10],
...
@app.route('/fixedepositp', methods=['POST', 'GET'])
def fixedepositp():
    return render_template('fixedeposit.html')

@app.route('/fixedeposit', methods=['POST', 'GET'])
def fixedeposit():

    client = MongoClient("mongodb://aviformat:sweswe@ds155634.mlab.com:55634/onlinebank")
    db = client.onlinebank
    cursor = db.customer.find({"id": session['username']}, {"balance": 1, "account_number": 1, "_id": 0})
    balance=0
    rate = 0
    for document in cursor:
        balance = document['balance']
    ...
    if amount < 10000:
        error = -1
        return render_template('fixedepositResult.html',result=error)

    if amount >= balance:
        error = -2
        return render_template('fixedepositResult.html',result=error,result2=balance)

    if period >= 10:
        rate = 9.26
        interest = (9.26/100)*amount
        totalinterest = interest*period
        maturity = amount + totalinterest        

    elif period < 10 and period >=5:
        rate = 8.16
        interest = (8.16/100)*amount
        totalinterest = interest*period
        maturity = amount + totalinterest
    
    elif period >=2 and period < 5:
        rate = 7.09
        interest = (7.09/100)*amount
        totalinterest = interest*period
        maturity = amount + totalinterest
    ...
    if error == 0:       
        ...
        db.customer.update_one({"id": session['username']}, {"$set": {"balance": balance}})
        t=time.ctime()
        date_after_month = datetime.now()+ relativedelta(years=period)
        balance = balance - amount

        db.fixedeposit.insert_one(
            {
                "Transaction Id": account_number[0:5] + time.strftime("%Y%m%d%H%M%S") + account_number[6:10],
                "account_number" : account_number,
                "Description": 'Online Fixed Deposit',
                "Initial Amount": amount,
                "Opening Date and Time": t,
                "Interest Rate" : rate,
                "Total Interest" : totalinterest,
                "Maturity Date" : date_after_month.strftime('%d/%m/%Y'),
                "Maturity Amount" : maturity,
                "Status": 'Successfull'
            }
        )

        # print account_number[0:5] + time.strftime("%Y%m%d%H%M%S") + account_number[6:10]

    return render_template('fixedepositResult.html',result=error,result1=interest,result2=totalinterest,result3=maturity,result4=amount,result5=date_after_month.strftime('%d/%m/%Y'),result6=t)


@app.route('/feedbackp', methods=['POST', 'GET'])
def feedbackp():
...
def feedback():
    ...
    db.feedback.insert_one(
            {
                "Feedback Id" : session['username'] + time.strftime("%Y%m%d%H%M%S"),
                "Customer Id" : session['username'],
...
@app.route('/reviewfeedback', methods=['POST', 'GET'])
def reviewfeedback():
    ...
    return render_template('feedbackall.html',result=cursor)
...
def summary():
    ...
    cursor1 = db.withdrawal.find({"Account_number": account_number})
    cursor2 = db.deposit.find({"Account_number": account_number})
    cursor3 = db.transfer.find({"Sender": account_number})
    cursor4 = db.fixedeposit.find({"account_number": account_number})

    return render_template('summary.html',result1=cursor1,result2=cursor2,result3=cursor3,result4=cursor4,result5=balance)

@app.route('/fixedepositSummary', methods=['POST', 'GET'])
def fixedepositSummary():
    client = MongoClient("mongodb://aviformat:sweswe@ds155634.mlab.com:55634/onlinebank")
    db = client.onlinebank

    cursor = db.fixedeposit.find({})

    return render_template('fixedepositReport.html',result=cursor)
